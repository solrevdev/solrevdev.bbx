...
public sealed class ListProjectUserPermissionsHandler(BitbucketClient client, CredentialManager credentials)
{
    public async Task<object> HandleAsync(ListProjectUserPermissionsRequest request, CancellationToken ct)
    {
        ...
        await foreach (var grant in client.GetPaginatedAsync<JsonElement>(
            $"workspaces/{ws}/projects/{key}/permissions-config/users", ct))
        {
