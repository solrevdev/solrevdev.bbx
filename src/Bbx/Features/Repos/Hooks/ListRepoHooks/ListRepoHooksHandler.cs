...
public sealed class ListRepoHooksHandler(BitbucketClient client, CredentialManager credentials)
{
    public async Task<object> HandleAsync(ListRepoHooksRequest request, CancellationToken ct)
    {
        ...
        return new { workspace = ws, repository = repo, count = hooks.Count, hooks };
    }
