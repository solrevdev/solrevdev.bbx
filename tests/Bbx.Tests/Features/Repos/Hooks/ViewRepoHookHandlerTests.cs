...
public class ViewRepoHookHandlerTests
{
    ...
    public async Task HandleAsync_escapes_uid_in_path()
    {
        ...
        await handler.HandleAsync(new ViewRepoHookRequest("ws", "myrepo", "{abc}"), CancellationToken.None);
    ...
    public async Task HandleAsync_projects_fields()
    {
        ...
        var result = (dynamic)await handler.HandleAsync(new ViewRepoHookRequest("ws", "myrepo", "{abc}"), CancellationToken.None);
